...
export default {
  ...
  data() {
    return {
      ...
      menuVisible: false,
      menuItemList: [
        { key: '1', icon: 'arrow-left', text: 'Fermer à gauche' },
        { key: '2', icon: 'arrow-right', text: 'Fermer à droite' },
        { key: '3', icon: 'close', text: 'Fermer les autres' },
      ],
      componentsArr: [],
    }
  },
  ...
  methods: {
    ...
    remove(key) {
      if (key == indexKey) {
        this.$message.warning("L'accueil ne peut pas être fermé !")
        return
      }
      if (this.pageList.length === 1) {
        this.$message.warning("C'est la dernière page, elle ne peut plus être fermée")
        return
      }

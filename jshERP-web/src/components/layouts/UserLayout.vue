    ...
    <div class="footer" v-if="device === 'desktop'">
      <div class="third-party-platform" v-if="isShowRight">
        ...
        <div class="platform-info" @click="openIPhone()">
          ...
          <span>Version iPhone</span>
        </div>
        <div style="width: 50px"></div>
        <div class="platform-info" @click="openMiniProgram()">
          <img src="/static/mini-program.png" style="height: 30px" />
          <span>Version Mini-programme</span>
        </div>
      </div>
      <p>
        <span v-if="this.isShowRight">GuanYiJia Technology</span>
        © 2015-2030 {{ systemTitle }} - Tous droits réservés
        <a style="color: #00458a; padding-right: 10px" :href="systemUrl" target="_blank">Site officiel</a>
        <span v-if="this.isShowRight"><a href="http://beian.miit.gov.cn/" target="_blank">苏ICP备2021042833号</a></span>
      </p>
    </div>
    <a-modal
      v-model="isAndroidShow"
      title="Scannez le code QR WeChat pour télécharger la version Android"
      width="200"
      centered
    >
      <template slot="footer">
        <a-button key="back" @click="handleAndroidCancel">Annuler</a-button>
      </template>
      <div class="platform-modal"><img src="/static/android-code.png" style="width: 200px" /></div>
    </a-modal>
    <a-modal
      v-model="isIphoneShow"
      title="Scannez le code QR WeChat pour télécharger la version iPhone"
      width="200"
      centered
    >
      <template slot="footer">
        <a-button key="back" @click="handleIphoneCancel">Annuler</a-button>
      </template>
      <div class="platform-modal"><img src="/static/iphone-code.png" style="width: 200px" /></div>
    </a-modal>
    <a-modal
      v-model="isMiniProgramShow"
      title="Scannez le code QR WeChat pour utiliser la version mini-programme"
      width="200"
      centered
    >
      <template slot="footer">
        <a-button key="back" @click="handleMiniProgramCancel">Annuler</a-button>
      </template>

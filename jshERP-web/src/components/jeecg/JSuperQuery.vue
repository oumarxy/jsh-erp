  ...
  <div class="j-super-query-box">
    <slot name="button" :isActive="superQueryFlag" :isMobile="izMobile" :open="handleOpen" :reset="handleReset">
      <a-tooltip v-if="superQueryFlag" v-bind="tooltipProps" :mouseLeaveDelay="0.2">
        ...
        <template slot="title">
          <span>Des conditions de requête avancée sont actives</span>
          <a-divider type="vertical" />
          <a @click="handleReset">Effacer</a>
        </template>
        <a-button-group>
      ...
      <a-button v-else type="primary" icon="filter" @click="handleOpen">Requête avancée</a-button>
    </slot>
    ...
    >
      <template slot="footer">
        <div style="float: left">
          <a-button :loading="loading" @click="handleReset">Réinitialiser</a-button>
          <a-button :loading="loading" @click="handleSave">Enregistrer les conditions de requête</a-button>
        </div>
        <a-button :loading="loading" @click="handleCancel">Fermer</a-button>
        <a-button :loading="loading" type="primary" @click="handleOk">Rechercher</a-button>
      </template>
          ...
          <a-col :sm="24" :md="24 - 5">
            <a-empty v-if="queryParamsModel.length === 0" style="margin-bottom: 12px">
              <div slot="description">
                <span>Aucune condition de requête</span>
                <a-divider type="vertical" />
                <a @click="handleAdd">Cliquez pour ajouter</a>
              </div>
            </a-empty>
            ...
            <a-form v-else layout="inline">
              <a-row style="margin-bottom: 12px">
                <a-col :md="12" :xs="24">
                  <a-form-item
                    label="Correspondance des conditions de filtre"
                    :labelCol="{ md: 6, xs: 24 }"
                    :wrapperCol="{ md: 18, xs: 24 }"
                    style="width: 100%"
                  >
                    <a-select v-model="matchType" :getPopupContainer="(node) => node.parentNode" style="width: 100%">
                      <a-select-option value="and">ET (toutes les conditions doivent correspondre)</a-select-option>
                      <a-select-option value="or">OU (n'importe quelle condition correspond)</a-select-option>
                    </a-select>
                  </a-form-item>
                </a-col>
              </a-row>
              ...
              >
                <a-col :md="8" :xs="24" style="margin-bottom: 12px">
                  <a-tree-select
                    ...
                    placeholder="Sélectionner un champ de requête"
                    allowClear
                ...
                <a-col :md="4" :xs="24" style="margin-bottom: 12px">
                  <a-select
                    placeholder="Règle de correspondance"
                    :value="item.rule"
                    :getPopupContainer="(node) => node.parentNode"
                    @change="handleRuleChange(item, $event)"
                  >
                    <a-select-option value="eq">Égal à</a-select-option>
                    <a-select-option value="like">Contient</a-select-option>
                    <a-select-option value="right_like">Commence par</a-select-option>
                    <a-select-option value="left_like">Se termine par</a-select-option>
                    <a-select-option value="in">Dans</a-select-option>
                    <a-select-option value="ne">Différent de</a-select-option>
                    <a-select-option value="gt">Supérieur à</a-select-option>
                    <a-select-option value="ge">Supérieur ou égal à</a-select-option>
                    <a-select-option value="lt">Inférieur à</a-select-option>
                    <a-select-option value="le">Inférieur ou égal à</a-select-option>
                  </a-select>
                </a-col>
                ...
                <a-col :md="8" :xs="24" style="margin-bottom: 12px">
                  ...
                  <j-date
                    ...
                    v-model="item.val"
                    placeholder="Veuillez sélectionner une heure"
                    :show-time="true"
      ...
      <a-modal
        title="Veuillez saisir le nom d'enregistrement"
...
export default {
  ...
  methods: {
    ...
    handlePromptOk() {
      ...
      if (!value) {
        this.$message.warning("Le nom d'enregistrement ne peut pas être vide")
        return
      }
      // 取出查询条件

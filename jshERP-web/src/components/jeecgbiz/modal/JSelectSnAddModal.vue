...
export default {
  ...
  data() {
    return {
      ...
      categoryTree: [],
      columns: [
        { dataIndex: 'serialNumber', title: 'Numéros de série saisis', width: 100, align: 'left' },
        {
          tdataIndex: 'action',
          title: 'Opération',
          align: 'center',
  ...
  methods: {
    ...
    onAdd() {
      ...
      if (isExist) {
        this.$message.warning('Désolé, ce numéro de série a déjà été ajouté !')
      } else {
    ...
    onBatchAdd() {
      ...
      for (let i = 0; i < nameArr.length; i++) {
        ...
        if (isExist) {
          this.$message.warning('Désolé, le numéro de série ' + nameArr[i] + ' a déjà été ajouté !')
          return
        } else {
          this.checkDataSource.push(checkObj)
        }

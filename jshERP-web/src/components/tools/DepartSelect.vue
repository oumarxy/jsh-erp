<template>
  <a-modal :title="currTitle" :width="450" :visible="visible" :closable="false" :maskClosable="closable">
    <template slot="footer">
      <a-button v-if="closable" @click="close">Annuler</a-button>
      <a-button type="primary" @click="departOk">Confirmer</a-button>
    </template>
    ...
    <a-form>
      ...
      >
        <a-tooltip placement="topLeft">
          <template slot="title">
            <span>Vous appartenez à plusieurs départements, veuillez sélectionner votre département actuel</span>
          </template>
        ...
        <a-select
          ...
          placeholder="Veuillez sélectionner le département de connexion"
...
export default {
  ...
  props: {
    title: {
      type: String,
      default: 'Sélection de département',
      required: false,
    },
    closable: {
      type: Boolean,
        ...
        getAction(url).then((res) => {
          if (res.success) {
            ...
            if (this.currDepartName) {
              this.currTitle = 'Changement de département (Département actuel : ' + this.currDepartName + ')'
            }
          }
          resolve()
        })
    ...
    show() {
      ...
      this.loadDepartList().then(() => {
        ...
        if (!this.departList || this.departList.length <= 0) {
          this.$message.warning("Vous n'avez pas encore configuré les informations du département !")
          this.departClear()
        }
      })
    },

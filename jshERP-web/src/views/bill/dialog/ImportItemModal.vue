<template>
  ...
  >
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="Modèle">
          <span
            ><a :href="tmpUrl" target="_blank"><b>Modèle Excel de détails [Télécharger]</b></a></span
          >
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="Fichier">
          <a-upload
            name="file"
      ...
      if (info.file.status === 'done') {
        if (info.file.response) {
          if (info.file.response.code === 200) {
            this.$message.success('Importation réussie : ' + info.file.response.data.rows.length + ' lignes')
        ...
        this.$message.error(`Échec de l'importation du fichier : ${info.file.msg} `)
      }
    },
    setFileData() {
      return {

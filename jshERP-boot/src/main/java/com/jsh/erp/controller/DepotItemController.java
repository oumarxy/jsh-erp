...
public class DepotItemController {
    ...
    /**
     * Rechercher la liste des documents selon l'entrepôt et le produit
     * @param mId
     * @param request
     * @return
     */
    @GetMapping(value = "/findDetailByDepotIdsAndMaterialId")
    @ApiOperation(value = "Rechercher la liste des documents selon l'entrepôt et le produit")
    public String findDetailByDepotIdsAndMaterialId(
        ...
        if (list != null) {
            for (DepotItemVo4DetailByTypeAndMId d: list) {
                ...
                item.put("number", d.getNumber()); // Numéro
                item.put("barCode", d.getBarCode()); // Code-barres
                item.put("materialName", d.getMaterialName()); // Nom
                String type = d.getType();
                String subType = d.getSubType();
                if(BusinessConstants.DEPOTHEAD_TYPE_OTHER.equals(type)) {
                    item.put("type", subType); // Type entrée/sortie
                } else {
                    item.put("type", subType + type); // Type entrée/sortie
                }
                item.put("depotName", d.getDepotName()); // Nom de l'entrepôt
                item.put("basicNumber", d.getBnum()); // Quantité
                item.put("unitPrice", d.getUnitPrice()); // Prix unitaire
                item.put("allPrice", d.getAllPrice()); // Montant
                item.put("operTime", Tools.getCenternTime(d.getOtime())); // Date
                dataArray.add(item);
            }
        }
        if (list == null) {
            ...
            return returnJson(objectMap, "Aucune donnée trouvée", ErpInfo.OK.code);
        }
    ...
    /**
     * Rechercher la quantité de stock selon le code-barres du produit et l'ID de l'entrepôt
     * @param depotId
     * @param barCode
     * @param request
     * @return
    ...
    @ApiOperation(value = "Rechercher la quantité de stock selon le code-barres du produit et l'ID de l'entrepôt")
    public BaseResponseInfo findStockByDepotAndBarCode(
        ...
        try {
            ...
            res.code = 500;
            res.data = "Échec de la récupération des données";
        }
        return res;
    }

    /**
     * Liste des détails de document
     * @param headerId
     * @param mpList
     * @param request
     * @return
    ...
    @ApiOperation(value = "Liste des détails de document")
    public BaseResponseInfo getDetailList(@RequestParam("headerId") Long headerId,
        ...
        try {
            ...
            // Tableau JSON pour stocker les données
            JSONArray dataArray = new JSONArray();
            if (null != dataList) {
                ...
                for (DepotItemVo4WithInfoEx diEx : dataList) {
                    ...
                    BigDecimal stock;
                    Unit unitInfo = materialService.findUnit(diEx.getMaterialId()); // Récupérer les informations multi-unités
                    ...
                    item.put("preNumber", diEx.getOperNumber()); // Quantité d'origine
                    item.put("finishNumber", depotItemService.getFinishNumber(diEx.getMaterialExtendId(), diEx.getId(), diEx.getHeaderId(), unitInfo, materialUnit, linkType)); // Entré|Sorti
                    item.put("purchaseDecimal", roleService.parseBillPriceByLimit(diEx.getPurchaseDecimal(), billCategory, priceLimit, request));  // Prix d'achat
                    if("basic".equals(linkType) || "1".equals(isReadOnly)) {
                        // Afficher le montant dans le cas normal, mais ne pas afficher le montant dans le cas d'achat basé sur la vente
    ...
    /**
     * Requête de statistiques d'entrée/sortie de stock
     * @param currentPage
     * @param pageSize
     * @param depotIds
     * @param beginTime
    ...
    @ApiOperation(value = "Requête de statistiques d'entrée/sortie de stock")
        ...
        try {
            ...
            if (null != dataList) {
                for (DepotItemVo4WithInfoEx diEx : dataList) {
                    ...
                    item.put("unitName", null!=diEx.getUnitId() ? diEx.getMaterialUnit()+"[Multi-unités]" : diEx.getMaterialUnit());
                    ...
                    // Convertir le stock de la petite unité en stock de la grande unité
    ...
    /**
     * Montant total des statistiques d'entrée/sortie de stock
     * @param depotIds
     * @param endTime
     * @param materialParam
     * @param request
    ...
    @ApiOperation(value = "Montant total des statistiques d'entrée/sortie de stock")
    ...
    /**
     * Rechercher la distribution du stock selon l'entrepôt et le produit - avec paramètres de période
     * @param mId
     * @param request
     * @return
     */
    @GetMapping(value = "/getMaterialDepotStockByParam")
    @ApiOperation(value = "Rechercher la distribution du stock selon l'entrepôt et le produit - avec paramètres de période")
    ...
    private List<Long> parseListByDepotIds(@RequestParam("depotIds") String depotIds) throws Exception {
        ...
        if(StringUtil.isNotEmpty(depotIds)) {
            ...
            // Par défaut, utiliser les entrepôts auxquels l'utilisateur actuel a accès lorsqu'aucun entrepôt n'est sélectionné
            JSONArray depotArr = depotService.findDepotByCurrentUser();
            for(Object obj: depotArr) {
                JSONObject object = JSONObject.parseObject(obj.toString());
                depotList.add(object.getLong("id"));
            }
            // Si le nombre d'entrepôts accessibles est trop élevé, demander de sélectionner un entrepôt
    ...
    @ApiOperation(value = "Statistiques d'achat")
    public BaseResponseInfo buyIn(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            if (null != dataList) {
                for (DepotItemVo4WithInfoEx diEx : dataList) {
                    JSONObject item = new JSONObject();
                    BigDecimal InSum = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_IN, BusinessConstants.SUB_TYPE_PURCHASE, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "number");
                    BigDecimal OutSum = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_OUT, BusinessConstants.SUB_TYPE_PURCHASE_RETURN, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "number");
                    BigDecimal InSumPrice = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_IN, BusinessConstants.SUB_TYPE_PURCHASE, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "price");
                    BigDecimal OutSumPrice = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_OUT, BusinessConstants.SUB_TYPE_PURCHASE_RETURN, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "price");
                    ...
                    item.put("inOutSumPrice",InOutSumPrice);// Montant d'achat réel
                    dataArray.add(item);
                }
            }
            BigDecimal inSumPriceTotal = depotItemService.buyOrSalePriceTotal(BusinessConstants.DEPOTHEAD_TYPE_IN, BusinessConstants.SUB_TYPE_PURCHASE, StringUtil.toNull(materialParam),
                    beginTime, endTime, creatorArray, organId, organArray, categoryList, depotList, forceFlag);
            BigDecimal outSumPriceTotal = depotItemService.buyOrSalePriceTotal(BusinessConstants.DEPOTHEAD_TYPE_OUT, BusinessConstants.SUB_TYPE_PURCHASE_RETURN, StringUtil.toNull(materialParam),
                    beginTime, endTime, creatorArray, organId, organArray, categoryList, depotList, forceFlag);
    ...
    /**
     * Statistiques de vente au détail
     * @param currentPage
     * @param pageSize
     * @param beginTime
     * @param endTime
    ...
    @ApiOperation(value = "Statistiques de vente au détail")
    public BaseResponseInfo retailOut(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            if (null != dataList) {
                for (DepotItemVo4WithInfoEx diEx : dataList) {
                    JSONObject item = new JSONObject();
                    BigDecimal OutSumRetail = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_OUT, BusinessConstants.SUB_TYPE_RETAIL, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "number");
                    BigDecimal InSumRetail = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_IN, BusinessConstants.SUB_TYPE_RETAIL_RETURN, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "number");
                    BigDecimal OutSumRetailPrice = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_OUT, BusinessConstants.SUB_TYPE_RETAIL, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "price");
                    BigDecimal InSumRetailPrice = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_IN, BusinessConstants.SUB_TYPE_RETAIL_RETURN, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "price");
                    ...
                    item.put("outInSumPrice",OutInSumPrice);// Montant de vente réel
                    dataArray.add(item);
                }
            }
            BigDecimal outSumPriceTotal = depotItemService.buyOrSalePriceTotal(BusinessConstants.DEPOTHEAD_TYPE_OUT, BusinessConstants.SUB_TYPE_RETAIL, StringUtil.toNull(materialParam),
                    beginTime, endTime, creatorArray, organId, organArray, categoryList, depotList, forceFlag);
            BigDecimal inSumPriceTotal = depotItemService.buyOrSalePriceTotal(BusinessConstants.DEPOTHEAD_TYPE_IN, BusinessConstants.SUB_TYPE_RETAIL_RETURN, StringUtil.toNull(materialParam),
                    beginTime, endTime, creatorArray, organId, organArray, categoryList, depotList, forceFlag);
    ...
    @ApiOperation(value = "Statistiques de vente")
    public BaseResponseInfo saleOut(@RequestParam("currentPage") Integer currentPage,
        ...
        try {
            ...
            String [] organArray = depotHeadService.getOrganArray(BusinessConstants.SUB_TYPE_SALES, "");
            List<Long> categoryList = new ArrayList<>();
            if(categoryId != null){
                categoryList = materialService.getListByParentId(categoryId);
            }
            ...
            if (null != dataList) {
                for (DepotItemVo4WithInfoEx diEx : dataList) {
                    JSONObject item = new JSONObject();
                    BigDecimal OutSum = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_OUT, BusinessConstants.SUB_TYPE_SALES, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "number");
                    BigDecimal InSum = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_IN, BusinessConstants.SUB_TYPE_SALES_RETURN, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "number");
                    BigDecimal OutSumPrice = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_OUT, BusinessConstants.SUB_TYPE_SALES, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "price");
                    BigDecimal InSumPrice = depotItemService.buyOrSale(BusinessConstants.DEPOTHEAD_TYPE_IN, BusinessConstants.SUB_TYPE_SALES_RETURN, diEx.getMaterialExtendId(), beginTime, endTime, creatorArray, organId, organArray, depotList, forceFlag, "price");
            ...
            BigDecimal outSumPriceTotal = depotItemService.buyOrSalePriceTotal(BusinessConstants.DEPOTHEAD_TYPE_OUT, BusinessConstants.SUB_TYPE_SALES, StringUtil.toNull(materialParam),
                    beginTime, endTime, creatorArray, organId, organArray, categoryList, depotList, forceFlag);
            BigDecimal inSumPriceTotal = depotItemService.buyOrSalePriceTotal(BusinessConstants.DEPOTHEAD_TYPE_IN, BusinessConstants.SUB_TYPE_SALES_RETURN, StringUtil.toNull(materialParam),
                    beginTime, endTime, creatorArray, organId, organArray, categoryList, depotList, forceFlag);
            BigDecimal realityPriceTotal = outSumPriceTotal.subtract(inSumPriceTotal);
            map.put("rows", dataArray);
            map.put("realityPriceTotal", realityPriceTotal);
    ...
    /**
     * Rapport d'alerte de stock
     * @param currentPage
     * @param pageSize
     * @return
     */
    @GetMapping(value = "/findStockWarningCount")
    @ApiOperation(value = "Rapport d'alerte de stock")
    ...
    /**
     * Statistiques des montants totaux d'achat, de vente et de vente au détail
     * @param request
     * @param response
     * @return
     * @throws Exception
    ...
    @ApiOperation(value = "Statistiques des montants totaux d'achat, de vente et de vente au détail")
    public BaseResponseInfo buyOrSalePrice(HttpServletRequest request,
        ...
        try {
            ...
            if(!"admin".equals(loginName)) {
                ...
                for (String month : monthList) {
                    ...
                    for (InOutPriceVo item : inOrOutPriceList) {
                        ...
                        if (month.equals(billOperMonth)) {
                            if (BusinessConstants.DEPOTHEAD_TYPE_IN.equals(item.getType()) && BusinessConstants.SUB_TYPE_PURCHASE.equals(item.getSubType())) {
                                outPrice = outPrice.add(item.getDiscountLastMoney());
                            }
                            if (BusinessConstants.DEPOTHEAD_TYPE_OUT.equals(item.getType()) && BusinessConstants.SUB_TYPE_PURCHASE_RETURN.equals(item.getSubType())) {
                ...
                for (String month : monthList) {
                    ...
                    for (InOutPriceVo item : inOrOutPriceList) {
                        String billOperMonth = Tools.dateToStr(item.getOperTime(), "yyyy-MM");
                        if (month.equals(billOperMonth)) {
                            if (BusinessConstants.DEPOTHEAD_TYPE_OUT.equals(item.getType()) && BusinessConstants.SUB_TYPE_SALES.equals(item.getSubType())) {
                                outPrice = outPrice.add(item.getDiscountLastMoney());
                            }
                            if (BusinessConstants.DEPOTHEAD_TYPE_IN.equals(item.getType()) && BusinessConstants.SUB_TYPE_SALES_RETURN.equals(item.getSubType())) {
                                inPrice = inPrice.add(item.getDiscountLastMoney());
                            }
                        }
                    }
                ...
                for (String month : monthList) {
                    ...
                    for (InOutPriceVo item : inOrOutPriceList) {
                        String billOperMonth = Tools.dateToStr(item.getOperTime(), "yyyy-MM");
                        if (month.equals(billOperMonth)) {
                            if (BusinessConstants.DEPOTHEAD_TYPE_OUT.equals(item.getType()) && BusinessConstants.SUB_TYPE_RETAIL.equals(item.getSubType())) {
                                outPrice = outPrice.add(item.getTotalPrice().abs());
                            }
                            if (BusinessConstants.DEPOTHEAD_TYPE_IN.equals(item.getType()) && BusinessConstants.SUB_TYPE_RETAIL_RETURN.equals(item.getSubType())) {
            ...
            logger.error(e.getMessage(), e);
            res.code = 500;
            res.data = "Statistique échouée";
        }
        return res;
    }

    /**
     * Obtenir les informations de la liste des produits par lot
     * @param request
     * @return
     */
    @GetMapping(value = "/getBatchNumberList")
    @ApiOperation(value = "Obtenir les informations de la liste des produits par lot")
    ...
    /**
     * Importer les détails depuis Excel
     * @param file
     * @param request
     * @param response
     * @return
    ...
    public BaseResponseInfo importItemExcel(MultipartFile file,
        ...
        try {
            String barCodes = "";
            // L'extension de fichier ne peut être que xls
            ...
            if(src.getRows()>1000) {
                message = "Échec de l'importation, les détails ne peuvent pas dépasser 1000 lignes";
                res.code = 500;
            ...
            logger.error(e.getMessage(), e);
            message = "Échec de l'importation, veuillez vérifier le contenu du tableau";
            res.code = 500;
            data.put("message", message);
            res.data = data;
        }

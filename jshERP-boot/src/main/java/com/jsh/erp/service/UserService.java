...
public class UserService {
    ...
    public User getUser(long id)throws Exception {
        User result=null;
        try{
            // Vérifier d'abord si l'utilisateur est connecté, puis rechercher les données utilisateur
    ...
    public int insertUser(JSONObject obj, HttpServletRequest request)throws Exception {
        ...
        // Comme le mot de passe est chiffré en MD5, il faut convertir le mot de passe
        try {
    ...
    public int updateUser(JSONObject obj, HttpServletRequest request) throws Exception{
        ...
        int result=0;
        try{
            // Vérifier si l'utilisateur s'est déjà connecté
    ...
    public int batDeleteUser(String ids, HttpServletRequest request) throws Exception{
        ...
        for(User user: list){
            if(user.getId().equals(user.getTenantId())) {
                logger.error("Code d'erreur[{}], Message d'erreur[{}], Paramètres, ids:[{}]",
        ...
        try{
            ...
            if (userId != null) {
                ...
                if(result>0) {
                    // Supprimer l'état de connexion de ces utilisateurs de Redis
    ...
    public void validateCaptcha(String code, String uuid) throws Exception {
        ...
        if(platformConfig!=null && "1".equals(platformConfig.getPlatformValue())) {
            if(StringUtil.isNotEmpty(code) && StringUtil.isNotEmpty(uuid)) {
                ...
                if (captcha == null) {
                    logger.error("Code d'erreur[{}], Message d'erreur[{}]", ExceptionConstants.USER_JCAPTCHA_EXPIRE_CODE, ExceptionConstants.USER_JCAPTCHA_EXPIRE_MSG);
                ...
                if (!code.equalsIgnoreCase(captcha)) {
                    logger.error("Code d'erreur[{}], Message d'erreur[{}]", ExceptionConstants.USER_JCAPTCHA_ERROR_CODE, ExceptionConstants.USER_JCAPTCHA_ERROR_MSG);
                ...
                logger.error("Code d'erreur[{}], Message d'erreur[{}]", ExceptionConstants.USER_JCAPTCHA_EMPTY_CODE, ExceptionConstants.USER_JCAPTCHA_EMPTY_MSG);
    ...
    public Map<String, Object> login(String loginName, String password, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            logger.error(">>>>>>>>>>>>>Utilisateur  " + loginName + " connexion méthode login accès couche service exception====", e);
        ...
        switch (userStatus) {
            ...
            case ExceptionCodeConstants.UserExceptionCode.USER_CONDITION_FIT:
                ...
                // Vérification réussie, peut se connecter, mettre en session, enregistrer le journal de connexion
        ...
        if(user!=null){
            ...
            if(BusinessConstants.DEFAULT_MANAGER.equals(user.getLoginName())) {
                // Si c'est un administrateur, envoyer un email de connexion
    ...
    public int validateUser(String loginName, String password) throws Exception {
        ...
        try {
            ...
            logger.error(">>>>>>>>Accès vérification nom utilisateur existe informations backend exception", e);
        ...
        try {
            ...
            logger.error(">>>>>>>>>>Accès vérification mot de passe utilisateur informations backend exception", e);
            return ExceptionCodeConstants.UserExceptionCode.USER_ACCESS_EXCEPTION;
        }
    ...
    public void addUserAndOrgUserRel(UserEx ue, HttpServletRequest request) throws Exception{
        if(BusinessConstants.DEFAULT_MANAGER.equals(ue.getLoginName())) {
            ...
            logService.insertLog("用户",
                    ...
                    ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest());
            // Vérifier le nom d'utilisateur et le nom de connexion
            checkLoginName(ue);
            // Ajouter les informations utilisateur
            ue= this.addUser(ue);
            if(ue==null){
                logger.error("Code d'erreur[{}], Message d'erreur[{}], Paramètres, [{}]",
            ...
            // ID utilisateur, rechercher l'ID selon le nom d'utilisateur
            ...
            if(ue.getOrgaId()==null){
                // Si aucune organisation n'est sélectionnée, ne pas créer de relation entre organisation et utilisateur
                return;
            }
            if(ue.getOrgaId()!=null && "1".equals(ue.getLeaderFlag())){
                // Vérifier si l'organisation actuelle a un manager
            ...
            // Ajouter la relation entre utilisateur et organisation
            OrgaUserRel oul=new OrgaUserRel();
            // ID de l'organisation
            oul.setOrgaId(ue.getOrgaId());
            oul.setUserId(userId);
            // Ordre de l'utilisateur dans l'organisation
    ...
    public void registerUser(UserEx ue, Integer manageRoleId, HttpServletRequest request) throws Exception{
        ...
        if(BusinessConstants.DEFAULT_MANAGER.equals(ue.getLoginName())) {
            ...
            // Mettre à jour l'ID du locataire
            User user = new User();
            user.setId(ue.getId());
            user.setTenantId(ue.getId());
            userService.updateUserTenant(user);
            // Ajouter la relation entre utilisateur et rôle
            JSONObject ubObj = new JSONObject();
            ubObj.put("type", "UserRole");
            ...
            // Créer les informations du locataire
            ...
            if(tenant.getUserNumLimit()==null) {
                tenant.setUserNumLimit(userNumLimit); // Nombre limite d'utilisateurs par défaut
            }
            if(tenant.getExpireTime()==null) {
                tenant.setExpireTime(Tools.addDays(new Date(), tryDayLimit)); // Nombre de jours d'essai autorisés pour le locataire
            }
            tenantMapper.insertSelective(tenant);
            logger.info("===============Création des informations du locataire terminée===============");
        }
    }
    ...
    public void updateUserAndOrgUserRel(UserEx ue, HttpServletRequest request) throws Exception{
        if(BusinessConstants.DEFAULT_MANAGER.equals(ue.getLoginName())) {
            ...
            // Mettre à jour la relation entre utilisateur et organisation
            OrgaUserRel oul = new OrgaUserRel();
            // ID de la relation entre organisation et utilisateur
            ...
            oul.setUserBlngOrgaDsplSeq(ue.getUserBlngOrgaDsplSeq());
            if (oul.getId() != null) {
                // La relation entre organisation et utilisateur existe déjà, mettre à jour
                oul = orgaUserRelService.updateOrgaUserRel(oul);
            } else {
                // La relation entre organisation et utilisateur n'existe pas, créer
                oul = orgaUserRelService.addOrgaUserRel(oul);
            }
            if (oul == null) {
                logger.error("Code d'erreur[{}], Message d'erreur[{}], Paramètres, [{}]",
    ...
    public void checkLoginName(UserEx userEx)throws Exception{
        ...
        if(!StringUtils.isEmpty(userEx.getLoginName())){
            ...
            if(list!=null&&list.size()>0){
                if(list.size()>1){
                    // Plus d'une donnée existe, ce nom de connexion existe déjà
                    logger.error("Code d'erreur[{}], Message d'erreur[{}], Paramètres, loginName:[{}]",
                ...
                // Une donnée, lancer une exception lors de l'ajout, lancer une exception lors de la modification si l'ID est différent de l'ID actuel
                if(list.size()==1){
    ...
    public JSONArray getBtnStrArrById(Long userId) throws Exception {
        ...
        if(userRoleList!=null && userRoleList.size()>0) {
            ...
            if(StringUtil.isNotEmpty(roleValue) && roleValue.indexOf("[")>-1 && roleValue.indexOf("]")>-1){
                roleValue = roleValue.replace("[", "").replace("]", ""); // ID du rôle - unique
        ...
        // Convertir les funId du tableau en URL
    ...
    public int batchSetStatus(Byte status, String ids, HttpServletRequest request)throws Exception {
        ...
        // Rechercher le nombre d'utilisateurs avec statut activé
        ...
        for(User user: list) {
            if(user.getId().equals(user.getTenantId())) {
                // Le locataire ne peut pas être désactivé
            } else {

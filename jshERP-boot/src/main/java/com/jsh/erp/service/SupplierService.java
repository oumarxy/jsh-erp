...
public class SupplierService {
    ...
    public List<Supplier> select(String supplier, String type, String contacts, String phonenum, String telephone) throws Exception{
        ...
        try{
            ...
            for(Supplier s : list) {
                ...
                String subTypeBack = "";
                String billType = "";
                if (BusinessConstants.SUPPLIER_TYPE_VENDOR.equals(supplierType)) {
                    inOutType = BusinessConstants.DEPOTHEAD_TYPE_IN;
                    ...
                    billType = "付款";
                } else if (BusinessConstants.SUPPLIER_TYPE_CUSTOMER.equals(supplierType)) {
                    inOutType = BusinessConstants.DEPOTHEAD_TYPE_OUT;
                ...
                if(BusinessConstants.SUPPLIER_TYPE_CUSTOMER.equals(s.getType())) {
                    s.setAllNeedGet(sum);
                } else if(BusinessConstants.SUPPLIER_TYPE_VENDOR.equals(s.getType())) {
                    s.setAllNeedPay(sum);
                }
            }
        } catch(Exception e){
    ...
    public int batchDeleteSupplierByIds(String ids)throws Exception {
        ...
        if(accountHeadList!=null&&accountHeadList.size()>0){
            logger.error("Code d'erreur[{}], Message d'erreur[{}], Paramètres, OrganIds[{}]",
    ...
    public List<Supplier> findBySelectCus(String key, Long organId, Integer limit)throws Exception {
        List<Supplier> list=null;
        try{
            list = supplierMapperEx.findByTypeAndKey(BusinessConstants.SUPPLIER_TYPE_CUSTOMER, key, limit);
            if(organId!=null) {
    ...
    public List<Supplier> findBySelectSup(String key, Long organId, Integer limit)throws Exception {
        List<Supplier> list=null;
        try{
            list = supplierMapperEx.findByTypeAndKey(BusinessConstants.SUPPLIER_TYPE_VENDOR, key, limit);
            if(organId!=null) {
                list = addOrganToList(list, organId);
            }
        }catch(Exception e){
    ...
    public List<Supplier> findBySelectRetail(String key, Long organId, Integer limit)throws Exception {
        List<Supplier> list=null;
        try{
            list = supplierMapperEx.findByTypeAndKey(BusinessConstants.SUPPLIER_TYPE_MEMBER, key, limit);
            if(organId!=null) {
                list = addOrganToList(list, organId);
            }
        }catch(Exception e){
    ...
    public List<Supplier> findUserCustomer()throws Exception{
        SupplierExample example = new SupplierExample();
        example.createCriteria().andTypeEqualTo(BusinessConstants.SUPPLIER_TYPE_CUSTOMER)
                .andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
        example.setOrderByClause("sort asc, id desc");
        List<Supplier> list=null;
        try{
    ...
    public Map<String, Object> getBeginNeedByOrganId(Long organId) throws Exception {
        ...
        if(BusinessConstants.SUPPLIER_TYPE_VENDOR.equals(supplier.getType())) {
            needDebt = supplier.getBeginNeedPay();
        } else if(BusinessConstants.SUPPLIER_TYPE_CUSTOMER.equals(supplier.getType())) {
    ...
    public void importVendor(MultipartFile file, HttpServletRequest request) throws Exception{
        String type = BusinessConstants.SUPPLIER_TYPE_VENDOR;
        User userInfo = userService.getCurrentUser();
    ...
    public void importCustomer(MultipartFile file, HttpServletRequest request) throws Exception{
        String type = BusinessConstants.SUPPLIER_TYPE_CUSTOMER;
        User userInfo = userService.getCurrentUser();
        Workbook workbook = Workbook.getWorkbook(file.getInputStream());
        Sheet src = workbook.getSheet(0);
    ...
    public void importMember(MultipartFile file, HttpServletRequest request) throws Exception{
        String type = BusinessConstants.SUPPLIER_TYPE_MEMBER;
        User userInfo = userService.getCurrentUser();
        Workbook workbook = Workbook.getWorkbook(file.getInputStream());
        Sheet src = workbook.getSheet(0);
    ...
    public File exportExcel(List<Supplier> dataList, String type) throws Exception {
        if(BusinessConstants.SUPPLIER_TYPE_VENDOR.equals(type)) {
            return exportExcelVendorOrCustomer(dataList, type);
        } else if(BusinessConstants.SUPPLIER_TYPE_CUSTOMER.equals(type)) {
            return exportExcelVendorOrCustomer(dataList, type);
        } else {
            //会员
            String[] names = {BusinessConstants.EXCEL_COL_MEMBER_CARD, BusinessConstants.EXCEL_COL_CONTACTS, BusinessConstants.EXCEL_COL_PHONE, BusinessConstants.EXCEL_COL_TELEPHONE, BusinessConstants.EXCEL_COL_EMAIL, BusinessConstants.EXCEL_COL_REMARK, BusinessConstants.EXCEL_COL_SORT, BusinessConstants.EXCEL_COL_STATUS};
            String title = BusinessConstants.EXCEL_TITLE_INFO;
    ...
    private File exportExcelVendorOrCustomer(List<Supplier> dataList, String type) throws Exception {
        String beginNeedStr = "";
        if(BusinessConstants.SUPPLIER_TYPE_VENDOR.equals(type)) {
            beginNeedStr = BusinessConstants.EXCEL_COL_BEGIN_NEED_PAY;
        } else if(BusinessConstants.SUPPLIER_TYPE_CUSTOMER.equals(type)) {
            beginNeedStr = BusinessConstants.EXCEL_COL_BEGIN_NEED_GET;
        }
        String[] names = {BusinessConstants.EXCEL_COL_NAME, BusinessConstants.EXCEL_COL_CONTACTS, BusinessConstants.EXCEL_COL_PHONE, BusinessConstants.EXCEL_COL_TELEPHONE, BusinessConstants.EXCEL_COL_EMAIL, BusinessConstants.EXCEL_COL_FAX, beginNeedStr,
                BusinessConstants.EXCEL_COL_TAX_NUM, BusinessConstants.EXCEL_COL_TAX_RATE, BusinessConstants.EXCEL_COL_BANK_NAME, BusinessConstants.EXCEL_COL_ACCOUNT_NUMBER, BusinessConstants.EXCEL_COL_ADDRESS, BusinessConstants.EXCEL_COL_REMARK, BusinessConstants.EXCEL_COL_SORT, BusinessConstants.EXCEL_COL_STATUS};
        String title = BusinessConstants.EXCEL_TITLE_INFO;
        List<Object[]> objects = new ArrayList<>();
        if (null != dataList) {
            for (Supplier s : dataList) {
    ...
    private void setUserCustomerPermission(HttpServletRequest request, Supplier supplier) throws Exception {
        if(BusinessConstants.SUPPLIER_TYPE_CUSTOMER.equals(supplier.getType())) {

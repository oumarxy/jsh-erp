...
public class OrganizationService {
    ...
    public int batchDeleteOrganizationByIds(String ids) throws Exception{
        ...
        if(organList!=null && organList.size()>0) {
            //如果存在子机构则不能删除
            logger.error("Code d'erreur[{}], Message d'erreur[{}]",
    ...
    public void checkOrgNoIsExists(String orgNo,Long id)throws Exception {
        ...
        if(orgList!=null&&orgList.size()>0){
            if(orgList.size()>1){
                logger.error("Code d'erreur[{}], Message d'erreur[{}], Paramètres, orgNo[{}]",
            ...
            if(id!=null){
                if(!orgList.get(0).getId().equals(id)){
                    ...
                    logger.error("Code d'erreur[{}], Message d'erreur[{}], Paramètres, orgNo[{}],id[{}]",
